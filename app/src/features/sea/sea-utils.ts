...
export function getSeaRateWithConversion(item: SeaRate, containerType: string) {
  const base = getSeaRateByContainerType(item, containerType);
  if (!base || base <= 0) return 0;
  const percent = parseConversionPercent(item.conversion);
